...
}`;const p=2,x=2,ie=100;class re{constructor(){t(this,"scene");t(this,"items",[]);const e=new m;this.scene=e.scene3;const n=[...new Array(ie)].map(()=>[Math.random()*p,Math.random()*x]),r=F.from(n).voronoi([0,0,p,x]);let c=0;const l=[];for(;;){const v=r.cellPolygon(c);if(v[0][0]==null)break;c++,l.push(v)}const a=new q({vertexShader:se,fragmentShader:oe,glslVersion:C,uniforms:{uMap:{value:null},uMatrix:{value:null},uOpacity:{value:1}},side:T,opacity:0,transparent:!0,wireframe:!1});l.forEach(v=>{v.shift();const u=this.makeGeom(v),d=new y(u.geom,a);d.position.x+=u.x-p/2,d.position.y+=u.y-x/2,this.scene.add(d);const o=O(d,{type:Q.HULL}).shape,h=new $({mass:1});h.addShape(o),h.angularVelocity.set(3,2,u.y),h.angularDamping=.8,e.physics.addBody(h),this.items.push({mesh:d,body:h,origin:{position:d.position.clone(),quaternion:d.quaternion.clone()}})}),this.apply()}setTexture(e){this.items.forEach(n=>{n.mesh.material.uniforms.uMap.value=e})}update(){this.items.forEach(e=>{e.mesh.position.set(e.body.position.x,e.body.position.y,e.body.position.z),e.mesh.quaternion.set(e.body.quaternion.x,e.body.quaternion.y,e.body.quaternion.z,e.body.quaternion.w)})}reset(){this.items.forEach(e=>{e.mesh.position.set(e.origin.position.x,e.origin.position.y,e.origin.position.z),e.mesh.quaternion.set(e.origin.quaternion.x,e.origin.quaternion.y,e.origin.quaternion.z,e.origin.quaternion.w),e.body.velocity.setZero(),e.body.position.set(e.mesh.position.x,e.mesh.position.y,e.mesh.position.z),e.body.quaternion.set(e.mesh.quaternion.x,e.mesh.quaternion.y,e.mesh.quaternion.z,e.mesh.quaternion.w)})}start(e,n){this.items.forEach(s=>{s.body.velocity.setZero();const r=s.mesh.position;console.log(r.x-e,r.y-n);const c=100;s.body.angularDamping=1,s.body.angularVelocity.set((r.x-e)*c,(r.y-n)*c,0)})}apply(){this.items.forEach(e=>{e.body.position.set(e.mesh.position.x,e.mesh.position.y,e.mesh.position.z),e.body.quaternion.set(e.mesh.quaternion.x,e.mesh.quaternion.y,e.mesh.quaternion.z,e.mesh.quaternion.w)})}makeGeom(e){const n=new G,s=[],r=[],c=[],l=.01,a=e.length,v=e.reduce((o,h)=>[o[0]+h[0],o[1]+h[1]],[0,0]),u=v[0]/a,d=v[1]/a;e.forEach(o=>{s.push(o[0]-u,o[1]-d,-l),r.push(o[0]/p,o[1]/x)}),e.forEach(o=>{s.push(o[0]-u,o[1]-d,l),r.push(o[0]/p,o[1]/x)});for(let o=1;o<a-1;o++)c.push(0,o,o+1);for(let o=1;o<a-1;o++)c.push(0+a,o+a,a+o+1);for(let o=0;o<a;o++){const h=o,z=(o+1)%a;c.push(h,z,h+a),c.push(z,z+a,h+a)}return n.setAttribute("position",new P(new Float32Array(s),3)),n.setAttribute("uv",new P(new Float32Array(r),2)),n.setIndex(c),{geom:n,x:u,y:d}}renderVoronoi(e){const n=document.createElement("canvas");document.body.appendChild(n),n.style.backgroundColor="#ccc",n.width=p,n.height=x;const s=n.getContext("2d");s.beginPath(),e.render(s),s.lineWidth=.25,s.strokeStyle="#ff0000",s.fillStyle="#ffff33",s.stroke()}}class ae{constructor(){t(this,"scene");t(this,"camera");t(this,"renderer");t(this,"mesh");const e=new m;this.scene=new g,this.camera=e.oCamera,this.renderer=e.renderer;const n=new f(2,2),s=new q({...K});this.mesh=new y(n,s),this.mesh.material.uniforms.resolution.value.x=1/(window.innerWidth*this.renderer.getPixelRatio()),this.mesh.material.uniforms.resolution.value.y=1/(window.innerHeight*this.renderer.getPixelRatio()),this.scene.add(this.mesh)}render(e,n){this.renderer.setRenderTarget(e),this.mesh.material.uniforms.tDiffuse.value=n,this.renderer.render(this.scene,this.camera),this.renderer.setRenderTarget(null)}}var ce=`uniform float uTime;
